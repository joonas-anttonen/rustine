...
public sealed partial class Log : IDisposable
{
    ...
    public record class Event(Severity Severity, string Message, DateTime Timestamp, string ThreadName, string Origin)
    {
        ...
        public string ToString(bool iso8601)
        {
            ...
            return $"[{ThreadName}]{(iso8601 ? iso8601Timestamp : "")}[{SeverityToString(Severity)}] {Origin} {Message}";
        }
    }
    ...
    /// </summary>
    public void Append(Severity severity, string message, string origin)
    {
        Event ev = new(
            Severity: severity,
            Message: message,
