...
pub fn register_ui_functions(lua: &mut LuaEngine) {
    ...
    lua.execute(
        ...
        -- UI namespace (immediate mode - still available)
        ui = {
            rect = ui_rect,
            text = ui_text,
...
//
// DOM Building API - Simplified approach using table parsing
//
...
/// Set the current DOM instance
pub fn set_current_dom(dom: crate::ui_dom::UiDom) {
    CURRENT_DOM.with(|d| {
        *d.borrow_mut() = Some(dom);
    });
}

/// Get a reference to the current DOM
pub fn with_current_dom<F, R>(f: F) -> Option<R>
where
    F: FnOnce(&mut crate::ui_dom::UiDom) -> R,
{
    CURRENT_DOM.with(|dom_cell| {
        dom_cell.borrow_mut().as_mut().map(|dom| f(dom))
    })
}

/// Clear the current DOM
pub fn clear_current_dom() {
    CURRENT_DOM.with(|d| {
        *d.borrow_mut() = None;
    });
}

/// Parse a UI node from a Lua table
pub fn parse_ui_node_from_lua(lua: &mut LuaEngine, table_name: &str) -> Option<UiNode> {
    // This is a simplified version - we'll enhance it to actually parse the table
    // For now, create a simple panel
    Some(UiNode::Panel {
...
/// Build DOM from Lua table structure
pub fn build_dom_from_lua(lua: &mut LuaEngine) -> Result<crate::ui_dom::UiDom, String> {
    // Call Lua's build_ui() function which returns a table structure
    let results = lua.call_function("build_ui", &[])?;
    
    // For now, create a simple DOM
    // TODO: Actually parse the Lua table
    let mut dom = crate::ui_dom::UiDom::new();
    
    // Try to get window size for layout
    let size_results = lua.call_function("ui.get_window_size", &[]).unwrap_or_default();
    let (w, h) = if size_results.len() >= 2 {
        if let (crate::lua::LuaValue::Number(w), crate::lua::LuaValue::Number(h)) = (&size_results[0], &size_results[1]) {
            (*w as f32, *h as f32)
        } else {
            (800.0, 600.0)
        }
        ...
        children: vec![
            UiNode::Text {
                ...
                style: StatefulStyle::new(Style {
                    ...
                    font_size: Some(1.2),
                    font_id: Some(gfx::fonts::CASKAYDIAMONO_FONT_ID),
                    ..Default::default()
                }),
                layout: Layout {
                    x: 20.0,
