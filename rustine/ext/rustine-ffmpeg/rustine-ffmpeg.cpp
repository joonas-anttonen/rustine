...
static rffmpeg_status rffmpeg_init_decoder_common(rffmpeg_decoder* dec, uint32_t* out_width, uint32_t* out_height,
												  uint32_t* out_frame_count, double* out_fps,
												  double* out_duration_sec);

extern "C" rffmpeg_status rffmpegDecoderCreate(const uint8_t* data, size_t size, rffmpeg_decoder** out_decoder,
											   uint32_t* out_width, uint32_t* out_height,
											   uint32_t* out_frame_count, double* out_fps,
											   double* out_duration_sec) {
	if (!data || size == 0 || !out_decoder || !out_width || !out_height || !out_frame_count || !out_fps ||
		!out_duration_sec) {
	...
	dec->avio_ctx = avio_alloc_context(dec->avio_buffer, avio_buffer_size, 0, dec, read_packet, nullptr, seek_packet);
	if (!dec->avio_ctx) {
		av_free(dec->avio_buffer);
		delete dec;
		return RFFMPEG_STATUS_ALLOCATION_FAILED;
	...
	rffmpeg_status status = rffmpeg_init_decoder_common(dec, out_width, out_height, out_frame_count, out_fps, out_duration_sec);
...
static rffmpeg_status rffmpeg_init_decoder_common(rffmpeg_decoder* dec, uint32_t* out_width, uint32_t* out_height,
	...
	if (video_stream->duration != AV_NOPTS_VALUE) {
		dec->duration_sec = static_cast<double>(video_stream->duration) * av_q2d(video_stream->time_base);
	...
	int rgba_buffer_size = av_image_get_buffer_size(AV_PIX_FMT_RGBA, dec->width, dec->height, 1);
	dec->rgba_buffer = static_cast<uint8_t*>(av_malloc(rgba_buffer_size));
	if (!dec->rgba_buffer) {
		rffmpeg_free_decoder_internal(dec);
		delete dec;
		return RFFMPEG_STATUS_ALLOCATION_FAILED;
	...
	// Setup RGBA frame
	av_image_fill_arrays(dec->rgba_frame->data, dec->rgba_frame->linesize, dec->rgba_buffer, AV_PIX_FMT_RGBA,
						 dec->width, dec->height, 1);
	
	// Create software scaler context for format conversion
	dec->sws_ctx =
		sws_getContext(dec->width, dec->height, dec->codec_ctx->pix_fmt, dec->width, dec->height, AV_PIX_FMT_RGBA,
...
extern "C" rffmpeg_status rffmpegDecoderCreateFromPath(const char* path, rffmpeg_decoder** out_decoder,
													   uint32_t* out_width, uint32_t* out_height,
													   uint32_t* out_frame_count, double* out_fps,
													   double* out_duration_sec) {
	if (!path || !out_decoder || !out_width || !out_height || !out_frame_count || !out_fps || !out_duration_sec) {
		return RFFMPEG_STATUS_INVALID_ARGUMENT;
	}
...
extern "C" rffmpeg_status rffmpegDecoderNext(rffmpeg_decoder* decoder, uint8_t* rgba_out, size_t rgba_capacity,
	...
	while (true) {
		...
		// Convert to RGBA
		sws_scale(decoder->sws_ctx, decoder->frame->data, decoder->frame->linesize, 0, decoder->height,
				  decoder->rgba_frame->data, decoder->rgba_frame->linesize);
		
		// Copy RGBA data to output buffer
		memcpy(rgba_out, decoder->rgba_buffer, required_size);
...
extern "C" rffmpeg_status rffmpegDecoderReset(rffmpeg_decoder* decoder) {
	...
	// Seek to beginning
	if (av_seek_frame(decoder->format_ctx, decoder->video_stream_index, 0, AVSEEK_FLAG_BACKWARD) < 0) {
		return RFFMPEG_STATUS_DECODE_FAILED;
	}
